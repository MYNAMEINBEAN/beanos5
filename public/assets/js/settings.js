...
const beanPremium = document.getElementById('bean-premium');
...
premiumActivate.addEventListener('click', async () => {
    if (beanPremium.value === '' || beanPremium.value === null || beanPremium.value === " ") {
...
adsDisable.addEventListener('click', () => {
    if (localStorage.getItem('a') === 'true') {
        localStorage.setItem('a', 'false');
        alert('Ads have been disabled. I guess you don`t want to support Bean. Meanie.');
        window.top.location.reload();
    } else if (localStorage.getItem('a') === 'false') {
        localStorage.setItem('a', 'true');
        alert('Ads have been re-enabled. Thanks for supporting Bean! 💖');
        window.top.location.reload();
    }
    else {
        localStorage.setItem('a', 'false');
